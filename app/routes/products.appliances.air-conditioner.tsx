...
export default function AirConditionerItems() {
  ...
  useEffect(() => {
    setPanelContent(selectedItem ? <ItemDetails item={selectedItem} /> : <p>No air conditioner items available.</p>)
  }, [selectedItem, setPanelContent])

  return (
    <div className="w-full p-4">
      <h1 className="text-xl font-bold mb-4">Air Conditioners</h1>
        ...
        {items.map((item) => (
          ...
          >
            ...
            <CardContent>
              ...
              <p className="text-gray-600">Price: ₹{item.price}</p>
              {item.capacity && <p className="text-gray-600">Capacity: {item.capacity}</p>}
              {item.energyRating && <p className="text-gray-600">Rating: {item.energyRating}★</p>}
            </CardContent>
          </button>
        ))}
      </div>
