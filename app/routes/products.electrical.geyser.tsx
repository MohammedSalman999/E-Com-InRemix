  ...
  return (
    <Card className="shadow-lg">
      ...
      <CardContent>
        ...
        {item.tankCap && <p className="text-gray-600">Tank Capacity: {item.tankCap}</p>}
        {item.heaterType && <p className="text-gray-600">Heater Type: {item.heaterType}</p>}
...
export default function GeyserItems() {
  ...
  useEffect(() => {
    setPanelContent(selectedItem ? <ItemDetails item={selectedItem} /> : <p>No geyser items available.</p>)
  }, [selectedItem, setPanelContent])

  return (
    <div className="w-full p-4">
      <h1 className="text-xl font-bold mb-4">Geysers</h1>
        ...
        {items.map((item) => (
          ...
          >
            ...
            <CardContent>
              <p className="text-gray-600">Brand: {item.brand}</p>
              <p className="text-gray-600">Price: ₹{item.price}</p>
              {item.tankCap && <p className="text-gray-600">Capacity: {item.tankCap}</p>}
            </CardContent>
          </button>
        ))}
      </div>
