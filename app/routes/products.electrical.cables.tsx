  ...
  return (
    <Card className="shadow-lg">
      ...
      <CardContent>
        ...
        {item.length && <p className="text-gray-600">Length: {item.length}</p>}
        {item.wireType && <p className="text-gray-600">Wire Type: {item.wireType}</p>}
        {item.wireColor && <p className="text-gray-600">Wire Color: {item.wireColor}</p>}
...
export default function CablesItems() {
  ...
  useEffect(() => {
    setPanelContent(selectedItem ? <ItemDetails item={selectedItem} /> : <p>No cable items available.</p>)
  }, [selectedItem, setPanelContent])

  return (
    <div className="w-full p-4">
      <h1 className="text-xl font-bold mb-4">Cables</h1>
