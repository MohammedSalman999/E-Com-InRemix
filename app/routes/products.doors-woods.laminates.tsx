...
// Server-side data fetching (Only LAMINATES category woods)
...
export default function LaminatesWoods() {
  ...
  useEffect(() => {
    setPanelContent(selectedWood ? <WoodDetails wood={selectedWood} /> : <p>No laminate materials available.</p>)
  }, [selectedWood, setPanelContent])

  return (
    <div className="w-full p-4">
      <h1 className="text-xl font-bold mb-4">Laminates</h1>
        ...
        {woods.map((wood) => (
          ...
          >
            ...
            <CardContent>
              ...
              {wood.finishes && <p className="text-gray-500">Finishes: {wood.finishes}</p>}
            </CardContent>
          </button>
        ))}
      </div>
